...
describe 'browser window' do
  ...
  describe "#close_browser" do
    before(:each) do
      @wt_temp = Rsel::SeleniumTest.new('http://localhost:8070/')
      @wt_temp.open_browser
    end

    it "returns true if there are no errors" do
      @wt_temp.close_browser('and show errors').should be_true
    end

    it "returns true if there are errors, but show_errors is unset" do
      @wt_temp.field_equals("Nonexistent field", "Nonexistent words")
      @wt_temp.close_browser('without showing errors').should be_true
    end

    it "raises StopTestStepFailed if there are errors and show_errors is set" do
      @wt_temp.field_equals("Nonexistent field", "Nonexistent words")
      lambda do
        @wt_temp.close_browser('and show errors')
