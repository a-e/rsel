  ...
  context "passes when" do
    ...
    context "text fields with labels in a hash" do
      ...
      it "sets many fields, some from a hash" do
        ...
        @st.field_contains("First name", "Ken").should be true
        @st.field_contains("Last name", "Brazier").should be true
        @st.field_contains("Life story", "testy").should be true
      end
    end
    it "clears a field" do
      @st.set_fields_among({"message" => ""},"").should be true
  ...
  context "fails when" do
    context "text fields with labels" do
      it "cant find the first field" do
        @st.set_fields_among({"Faust name" => "Ken", "Last name" => "Brazier"}).should be false
      end

      it "cant find the last field" do
        @st.set_fields_among({"First name" => "Ken", "Lost name" => "Brazier"}).should be false
      end
    end
    context "text fields with labels in a hash" do
      it "cant find the first field" do
